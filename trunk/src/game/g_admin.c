...
g_admin_cmd_t g_admin_cmds[ ] ={
  ...
  {"register", G_admin_register, "help",
  ...
  {
    "printgrid", G_admin_printgrid, "warn",
    "Print the node grid",
    "Dont fucking use it"
  },
  {
    "printpath", G_admin_printpath, "warn",
    "Print the node grid",
    "Dont fucking use it"
  },
  {
    "findpath", G_admin_findpath, "warn",
    "Find a path from where im to a node",
    "Dont fucking use it"
  },
  {
    "survivalstage", G_set_survival_stage, "warn",
    "Set & show survival stage",
...
qboolean G_admin_subnetban(gentity_t *ent, int skiparg) {
  ...
  if (mask > 0) {
    Q_strncpyz(
            cIPRlow,
            va("%hhu.%hhu.%hhu.%hhu", (IPRlow & (255 << 24)) >> 24, (IPRlow & (255 << 16)) >> 16, (IPRlow & (255 << 8)) >> 8, IPRlow & 255),
            sizeof ( cIPRlow)
            );
    Q_strncpyz(
            cIPRhigh,
            va("%hhu.%hhu.%hhu.%hhu", (IPRhigh & (255 << 24)) >> 24, (IPRhigh & (255 << 16)) >> 16, (IPRhigh & (255 << 8)) >> 8, IPRhigh & 255),
            sizeof ( cIPRhigh)
            );
  } else {
...
qboolean G_admin_printgrid(gentity_t *ent, int skiparg){
  ...
  xp = ((100/2)+(ent->s.origin[0]/100));
  yp = ((100/2)+(ent->s.origin[1]/100));
  ...
  xnode = ((100/2)+(node->s.origin[0]/100));
  ynode = ((100/2)+(node->s.origin[1]/100));
  //Prevent buffer overflow.
  if(xnode >= 100 || ynode >= 100 || xnode < 0 || ynode < 0)
  {
...
qboolean G_admin_findpath(gentity_t *ent, int skiparg){
  ...
  while(findway(0, xnode, ynode, x, y)) //Reversed from alien spawn to human
  {
    ...
    cleanvis();
    if(level.LEFT == min(min(level.LEFT,level.RIGHT), min(level.UP,level.DOWN)))
    {
    ...
    else if(level.RIGHT == min(min(level.LEFT,level.RIGHT),min(level.UP,level.DOWN)))
    {
    ...
    else if(level.UP == min(min(level.LEFT,level.RIGHT),min(level.UP,level.DOWN)))
    {
      G_LogPrintf(va("Go Up %d %d %d %d \n",level.RIGHT, level.maxpasos, x, y));
      ynode = ynode-1;
      pathpos++;
    }
    else if(level.DOWN == min(min(level.LEFT,level.RIGHT),min(level.UP,level.DOWN)))
    {
...
qboolean G_admin_warn(gentity_t *ent, int skiparg) {//mostly copy and paste with the proper lines altered from !mute and !kick
  int pids[ MAX_CLIENTS ];
  char name[ MAX_NAME_LENGTH ], *reason, err[ MAX_STRING_CHARS ];
  int minargc;
  gentity_t *vic;
