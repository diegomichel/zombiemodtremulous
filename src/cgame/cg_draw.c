 ...
 */
static void
CG_DrawFieldPadded(int x, int y, int width, int cw, int ch, int value)
{
  char num[16], *ptr;
  int l, orgL;
  int frame;
 ...
 */
static void
CG_DrawField(int x, int y, int width, int cw, int ch, int value)
{
  char num[16], *ptr;
  int l;
  int frame;
...
static void
CG_DrawPlayerCreditsValue(rectDef_t *rect, vec4_t color, qboolean padding)
{
  int value;
  playerState_t *ps;
  centity_t *cent;
...
static void
CG_DrawPlayerBankValue(rectDef_t *rect, vec4_t color, qboolean padding)
{
  int value;
  playerState_t *ps;
 ...
 */
static void
CG_DrawPlayerStamina1(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
 ...
 */
static void
CG_DrawPlayerStamina2(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  float stamina = ps->stats[STAT_STAMINA];
  float maxStaminaBy3 = (float) MAX_STAMINA / 3.0f;
 ...
 */
static void
CG_DrawPlayerStamina3(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  float stamina = ps->stats[STAT_STAMINA];
  float maxStaminaBy3 = (float) MAX_STAMINA / 3.0f;
 ...
 */
static void
CG_DrawPlayerStamina4(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  float stamina = ps->stats[STAT_STAMINA];
  float progress;
 ...
 */
static void
CG_DrawPlayerStaminaBolt(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  float stamina = ps->stats[STAT_STAMINA];
 ...
 */
static void
CG_DrawPlayerClipsRing(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  centity_t *cent;
 ...
 */
static void
CG_DrawPlayerBuildTimerRing(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  centity_t *cent;
  float buildTime = ps->stats[STAT_MISC];
 ...
 */
static void
CG_DrawPlayerBoosted(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
 ...
 */
static void
CG_DrawPlayerBoosterBolt(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  qboolean boosted = ps->stats[STAT_STATE] & SS_BOOSTED;
  vec4_t localColor;
 ...
 */
static void
CG_DrawPlayerPoisonBarbs(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
  int x = rect->x;
  int y = rect->y;
 ...
 */
static void
CG_DrawPlayerWallclimbing(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
...
static void
CG_DrawPlayerAmmoValue(rectDef_t *rect, vec4_t color)
{
  int value;
  centity_t *cent;
  playerState_t *ps;
 ...
 */
static void
CG_DrawUsableBuildable(rectDef_t *rect, qhandle_t shader, vec4_t color)
{
  vec3_t view, point;
  trace_t trace;
  entityState_t *es;
...
static void
CG_DrawPlayerBuildTimer(rectDef_t *rect, vec4_t color)
{
  float progress;
  int index;
  centity_t *cent;
...
static void
CG_DrawPlayerClipsValue(rectDef_t *rect, vec4_t color)
{
  int value;
  centity_t *cent;
  playerState_t *ps;
...
static void
CG_DrawPlayerHealthValue(rectDef_t *rect, vec4_t color)
{
  playerState_t *ps;
  int value;
 ...
 */
static void
CG_DrawPlayerHealthCross(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps = &cg.snap->ps;
...
static void
CG_DrawLevelShot(rectDef_t *rect)
{
  const char *s;
  const char *info;
  qhandle_t levelshot;
 ...
 */
static void
CG_DrawDemoPlayback(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
 ...
 */
static void
CG_DrawDemoRecording(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  if (!cg_drawDemoState.integer)
    return;
 ...
 */
void
CG_UpdateMediaFraction(float newFract)
{
 ...
 */
static void
CG_DrawTimerMins(rectDef_t *rect, vec4_t color)
{
  int mins, seconds;
  int msec;
 ...
 */
static void
CG_DrawTimerSecs(rectDef_t *rect, vec4_t color)
{
  int mins, seconds;
  int msec;
...
static void
CG_DrawZombieTimerBackground(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
}

static void
CG_DrawZombieRecordBackground(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
}

static void
CG_DrawExtraMediKits(rectDef_t *rect, vec4_t color, qhandle_t shader)
{
  playerState_t *ps;
  centity_t *cent;
  int kits;
  ...
  kits = ps->persistant[PERS_UNUSED];
...
static void
CG_DrawMedal(rectDef_t *rect, vec4_t color, qhandle_t shader, int medal)
{
  int currentRecord = -1;
  int offset = 0;
  ...
  if (cg.time - cgs.levelStartTime < cgs.survivalRecords[0]) //A simple operand can change everything.
  {
    currentRecord = -1; //Havent Win any medal.
  }
  else if (cg.time - cgs.levelStartTime < cgs.survivalRecords[1])
  {
    currentRecord = 0; //Bronze
  }
  else if (cg.time - cgs.levelStartTime < cgs.survivalRecords[2])
  {
    currentRecord = 1; //Silver
  }
  else
  {
    currentRecord = 2; //gold
  }

  /*if(currentRecord == -1)
   return;*/

  if (currentRecord < medal)
    return;

  if (medal == 0 && currentRecord == 0)
  {
    offset = 78;
  }
  if (currentRecord == 1)
  ...
  CG_DrawPic(rect->x - offset, rect->y, rect->w, rect->h, shader);
  trap_R_SetColor(NULL);
}
  ...
  int align, int textStyle, qhandle_t shader)
{
  ...
  int mins, seconds, tens, ms, ums, dms;
  int msec;
  int currentRecord;

  vec4_t textColor, adjustedColor;
  ...
  ms = msec - (mins * 60 * 1000) - (seconds * 1000) - (tens * 10 * 1000);
  
  dms = (ms / (999 / 99)) / 10;
  ums = (ms / (999 / 99)) % 10;

  s = va("%d%d:%d%d:%d%d", mins / 10, mins % 10, tens, seconds, dms, ums);
  ...
  if (currentRecord > -1)
  {
    if (cgs.survivalRecords[currentRecord] - (cg.time - cgs.levelStartTime) < 10000)
    {
      if (dms == 0)
      {
        cg.fadeTimer = cg.time + 1000;
      }

      if (cg.time < cg.fadeTimer)
      {
        color[3] = ((cg.fadeTimer) - cg.time) / (1000.0f);
      }
      if (color[3] == 0)
      {
        color[3] = 1;
  ...
  /*color = CG_FadeColor( fadeTimer, 1000 * cg_centertime.value );
   if( !color )
   return;*/
  ...
  int align, int textStyle, qhandle_t shader)
{
  ...
  //Some how this fucking piece of crap dont work... WHY? i dont know.
  ...
  if (msec < cgs.survivalRecords[0]) //A simple operand can change everything.
  {
    currentRecord = 0;
  }
  else if (msec < cgs.survivalRecords[1])
  {
    currentRecord = 1;
  }
  else if (msec < cgs.survivalRecords[2])
  {
    currentRecord = 2;
  }
  else
  ...
  ms = cgs.survivalRecords[currentRecord];
  seconds = cgs.survivalRecords[currentRecord] / 1000;
  mins = seconds / 60;
  seconds -= mins * 60;
  tens = seconds / 10;
  seconds -= tens * 10;
 ...
 */
static void
CG_DrawLagometer(rectDef_t *rect, float text_x, float text_y, float scale, vec4_t textColor)
{
  int a, x, y, i;
  float v;
  ...
  int textStyle, const char *text, menuDef_t *parent, itemDef_t *textItem)
{
  ...
  //Survival text must be shorter 290
 ...
 */
void
CG_DrawWeaponIcon(rectDef_t *rect, vec4_t color)
{
 ...
 */
static void
CG_DrawCrosshairNames(rectDef_t *rect, float scale, int textStyle)
{
  float *color;
  char *name;
  float w, x;
  ...
  int textStyle)
{
  ...
  switch(ownerDraw)
  {
    ...
    case CG_LAGOMETER:
      ...
      //zombieSurvival
    case CG_ZOMBIETIMER:
      CG_DrawZombieTimer(&rect, text_x, text_y, scale, color, align, textStyle, shader);
      break;
    case CG_ZOMBIERECORD:
      CG_DrawZombieRecord(&rect, text_x, text_y, scale, color, align, textStyle, shader);
      break;
      
    case CG_ZOMBIETIMERBACKGROUND:
      CG_DrawZombieTimerBackground(&rect, color, shader);
      break;
      
    case CG_ZOMBIERECORDBACKGROUND:
      CG_DrawZombieRecordBackground(&rect, color, shader);
      break;
      //Survival Medals
    case CG_GOLDMEDAL:
      CG_DrawMedal(&rect, color, shader, 2);
      break;

    case CG_SILVERMEDAL:
      CG_DrawMedal(&rect, color, shader, 1);
      break;
      
    case CG_BRONZEMEDAL:
      CG_DrawMedal(&rect, color, shader, 0);
      break;

    case CG_EXTRAMEDIKIT:
      CG_DrawExtraMediKits(&rect, color, shader);
      break;
...
CG_DrawLighting(void)
{
  ...
  if ((cg.snap->ps.stats[STAT_STAMINA] < -800) && (cg.snap->ps.stats[STAT_PTEAM] == PTE_HUMANS))
  {
    vec4_t black =
    { 0, 0, 0, 0 };
...
CG_DrawQueue(void)
{
  ...
  if (cg.snap->ps.stats[STAT_PTEAM] == PTE_ALIENS)
  {
    if (cgs.numAlienSpawns == 1)
      ...
      Com_sprintf(buffer, MAX_STRING_CHARS, "There are %d spawns remaining.", cgs.numAlienSpawns);
  }
  else if (cg.snap->ps.stats[STAT_PTEAM] == PTE_HUMANS)
  {
    if (cgs.numHumanSpawns == 1)
      ...
      Com_sprintf(buffer, MAX_STRING_CHARS, "There are %d spawns remaining.", cgs.numHumanSpawns);
  }
 ...
 */
static void
CG_ScalePainBlendTCs(float* s1, float *t1, float *s2, float *t2)
{
  *s1 -= 0.5f;
  *t1 -= 0.5f;
  *s2 -= 0.5f;
