 ...
 */
void
CG_RegisterUpgrade(int upgradeNum)
{
 ...
 */
static qboolean
CG_ParseWeaponModeSection(weaponInfoMode_t *wim, char **text_p)
{
  char *token;
  int i;
 ...
 */
static qboolean
CG_ParseWeaponFile(const char *filename, weaponInfo_t *wi)
{
  char *text_p;
  int len;
  char *token;
...
static float
CG_MachinegunSpinAngle(centity_t *cent, qboolean firing)
{
  int delta;
  float angle;
  float speed;
...
CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
  ...
  if (((cent->currentState.eFlags & EF_FIRING) && weaponMode == WPM_PRIMARY) || ((cent->currentState.eFlags & EF_FIRING2) && weaponMode == WPM_SECONDARY)
  ...
  if (ps || cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
  {
    ...
    {
      trap_R_AddLightToScene(flash.origin, 300 + (rand() & 31), weapon->wim[weaponMode].flashDlightColor[0], weapon->wim[weaponMode].flashDlightColor[1],
...
CG_AddViewWeapon(playerState_t *ps)
{
  ...
  if ((ps->persistant[PERS_TEAM] == TEAM_SPECTATOR) || (ps->stats[STAT_STATE] & SS_INFESTING) || (ps->stats[STAT_STATE] & SS_HOVELING))
    return;
 ...
 */
static qboolean
CG_WeaponSelectable(weapon_t weapon)
{
  //int ammo, clips;
  //
 ...
 */
static qboolean
CG_UpgradeSelectable(upgrade_t upgrade)
{
 ...
 */
void
CG_DrawItemSelect(rectDef_t *rect, vec4_t color)
{
  int i;
  int x = rect->x;
  int y = rect->y;
 ...
 */
void
CG_DrawItemSelectText(rectDef_t *rect, float scale, int textStyle)
{
  int x, w;
  char *name;
  float *color;
  ...
  {
    if (cg_weapons[cg.weaponSelect].registered && BG_InventoryContainsWeapon(cg.weaponSelect, cg.snap->ps.stats))
    {
  ...
  {
    if (cg_upgrades[cg.weaponSelect - 32].registered && BG_InventoryContainsUpgrade(cg.weaponSelect - 32, cg.snap->ps.stats))
    {
 ...
 */
void
CG_FireWeapon(centity_t *cent, weaponMode_t weaponMode)
{
  entityState_t *es;
  int c;
  weaponInfo_t *wi;
  ...
  {
    if (!CG_IsParticleSystemValid(&cent->muzzlePS) || !CG_IsParticleSystemInfinite(cent->muzzlePS))
 ...
 */
void
CG_MissileHitWall(weapon_t weaponNum, weaponMode_t weaponMode, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType)
{
  qhandle_t mark = 0;
  qhandle_t ps = 0;
  int c;
 ...
 */
void
CG_MissileHitPlayer(weapon_t weaponNum, weaponMode_t weaponMode, vec3_t origin, vec3_t dir, int entityNum)
{
  vec3_t normal;
  weaponInfo_t *weapon = &cg_weapons[weaponNum];
