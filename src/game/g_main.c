...
void G_InitGame(int levelTime, int randomSeed, int restart) {
  ...
  if(Q_stricmp(level.layout, "survival") == 0){
    ...
    //Fetch survival records from server...
      if(trap_mysql_runquery(va("SELECT time FROM games WHERE map = \"%s\" ORDER BY time desc LIMIT 3",
        map)) == qtrue)
      {
...
void G_addBot() {
  ...
  if(level.numAlienSpawns < 1) return;
...
void G_CalculateSurvivalRecords(void){
  int i;
  if(g_survival.integer)
  {
    for(i=0;i<3;i++)
...
void G_SendGameStat(pTeam_t team) {
  ...
  switch (team) {
    case PTE_ALIENS: teamChar = 'A';
      break;
    case PTE_HUMANS: teamChar = 'H';
      break;
    case PTE_NONE: teamChar = 'L';
      break;
    default: return;
  }
