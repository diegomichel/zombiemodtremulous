    ...
    g_admin_cmds[] =
        {
            ...
            { "allowbuild", G_admin_denybuild, "denybuild",
                "restore a player's ability to build", "[^3name|slot#^7]" },
            ...
            { "bot", G_admin_bot, "Z", "Add or delete bot(s)",
            ...
            { "denybuild", G_admin_denybuild, "denybuild",
                "take away a player's ability to build", "[^3name|slot#^7]" },
            ...
            { "info", G_admin_info, "info",
                "display the contents of server info files", "(^5subject^7)" },

            { "kick", G_admin_kick, "kick",
                "kick a player with an optional reason",
                "[^3name|slot#^7] (^5reason^7)" },

            { "L0", G_admin_L0, "l0", "Sets a level 1 to level 0",
                "[^3name|slot#^7]" },

            { "L1", G_admin_L1, "l1", "Sets a level 0 to level 1",
                "[^3name|slot#^7]" },

            { "layoutsave", G_admin_layoutsave, "layoutsave",
                "save a map layout", "[^3mapname^7]" },
            ...
            { "listmaps", G_admin_listmaps, "help",
                "display a list of available maps on the server",
                "(^5map name^7)" },

            { "lock", G_admin_lock, "lock",
                "lock a team to prevent anyone from joining it", "[^3a|h^7]" },
            ...
            { "rename", G_admin_rename, "rename", "rename a player",
            ...
            { "rotation", G_admin_listrotation, "help",
            ...
            { "undesignate", G_admin_designate, "designate",
                "revoke designated builder privileges", "[^3name|slot#^7]" },

            { "unlock", G_admin_unlock, "lock", "unlock a locked team",
                "[^3a|h^7]" },

            { "unmute", G_admin_mute, "mute", "unmute a muted player",
                "[^3name|slot#^7]" },

            { "warn", G_admin_warn, "warn",
                "Warn a player to cease or face admin intervention",
                "[^3name|slot#^7] [reason]" },
            { "reconnectdb", G_reconnectdb, "lock",
                "reconnect to the database", "do it!" },
            { "switch", G_admin_switch, "warn", "Switch your own team",
                "[^3name|slot#^7] [reason]" },
            { "printgrid", G_admin_printgrid, "warn", "Print the node grid",
                "Dont fucking use it" },
            { "printpath", G_admin_printpath, "warn", "Print the node grid",
                "Dont fucking use it" },
            { "findpath", G_admin_findpath, "warn",
                "Find a path from where im to a node", "Dont fucking use it" },
            { "survivalstage", G_set_survival_stage, "warn",
                "Set & show survival stage", "#Only use it when making layouts" },
            { "switchnodes", G_switchnodes, "warn",
                "Show and hide alien nodes", "Just dont use this shit" } };
...
admin_permission(char *flags, const char *flag, qboolean *perm)
{
	...
	{
		*perm = qtrue;
		if (*token == '-' || *token == '+') *perm = *token++ == '+';
		if (!strcmp(token, flag)) return qtrue;
		if (!strcmp(token, ADMF_ALLFLAGS))
		{
			while (*(token = COM_Parse(&token_p)))
			{
				// -ALLFLAGS +flag or ALLFLAGS -flag
				if (*token != (*perm ? '-' : '+')) continue;
...
G_admin_permission_guid(char *guid, const char* flag)
{
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++)
	{
		...
		{
			if (admin_permission(g_admin_admins[i]->flags, flag, &perm)) return perm;
	...
	{
		if (g_admin_levels[i]->level == l) return admin_permission(
		    g_admin_levels[i]->flags, flag, &perm) && perm;
	}
	return qfalse;
}
...
G_admin_name_check(gentity_t *ent, char *name, char *err, int len)
{
	...
	if (!Q_stricmp(name2, "UnnamedPlayer")) return qtrue;
	...
	for (i = 0; i < level.maxclients; i++)
	{
		...
		if (client->pers.connected != CON_CONNECTING && client->pers.connected
		    != CON_CONNECTED)
		{
			continue;
		}
	...
	for (i = 0; name2[i] != '\0'; i++)
	{
		if (Q_isalpha(name2[i])) alphaCount++;
	...
	if (!g_adminNameProtect.string[0]) return qtrue;

	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++)
	{
		if (g_admin_admins[i]->level < 1) continue;
		G_SanitiseString(g_admin_admins[i]->name, testName, sizeof(testName));
		if (!Q_stricmp(name2, testName) && Q_stricmp(ent->client->pers.guid,
...
admin_higher_guid(char *admin_guid, char *victim_guid)
{
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++)
	{
		...
		{
			if (alevel < g_admin_admins[i]->level) return qfalse;
			if (strstr(g_admin_admins[i]->flags, va("%s", ADMF_IMMUTABLE))) return qfalse;
		}
	}
	return qtrue;
}
...
admin_writeconfig(void)
{
	...
	for (i = 0; i < MAX_ADMIN_BANS && g_admin_bans[i]; i++)
	{
		...
		if (g_admin_bans[i]->expires != 0 && (g_admin_bans[i]->expires - t) < 1) continue;
...
admin_default_levels(void)
{
	...
	Q_strncpyz(g_admin_levels[5]->flags,
	    "ALLFLAGS -INCOGNITO -IMMUTABLE -DBUILDER -BANIMMUNITY", sizeof(l->flags));
}
...
G_admin_get_adminname(gentity_t *ent)
{
	...
	if (!ent) return "console";

	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++)
	{
		if (!Q_stricmp(g_admin_admins[i]->guid, ent->client->pers.guid)) return g_admin_admins[i]->name;
	}
	...
	*flags = '\0';
	...
	if (victim && Q_stricmp(cmd, "attempted"))
	{
		Com_sprintf(string, sizeof(string),
		    ...
		    (admin) ? admin->s.clientNum : -1, (admin) ? admin->client->pers.guid
		        : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
		    (admin) ? admin->client->pers.netname : "console",
		    (admin) ? admin->client->pers.adminName : "console", flags, cmd,
	...
	{
		Com_sprintf(string, sizeof(string), "%s^7 (%i) %s: %s",
		    (admin) ? admin->client->pers.netname : "console",
		    (admin) ? admin->s.clientNum : -1, cmd, G_SayConcatArgs(1 + skiparg));
		G_AdminsPrintf("%s\n", string);
	}
...
admin_listadmins(gentity_t *ent, int start, char *search, int minlevel)
{
	...
	for (i = 0; i < level.maxclients && search[0]; i++)
	{
		...
		if (vic->client && vic->client->pers.connected != CON_CONNECTED) continue;

		l = vic->client->pers.adminLevel;

		G_SanitiseString(vic->client->pers.netname, name, sizeof(name));
		if (!strstr(name, search)) continue;
		...
		for (j = 0; j < MAX_ADMIN_LEVELS && g_admin_levels[j]; j++)
		{
			if (g_admin_levels[j]->level == l)
			{
				...
				Com_sprintf(lname_fmt, sizeof(lname_fmt), "%%%is", (admin_level_maxname
				    + strlen(g_admin_levels[j]->name) - strlen(lname)));
	...
	for (i = start; i < MAX_ADMIN_ADMINS && g_admin_admins[i] && drawn
	    < MAX_ADMIN_LISTITEMS; i++)
			...
			for (j = 0; j < MAX_ADMIN_LEVELS && g_admin_levels[j]; j++)
			{
				if (g_admin_levels[j]->level == g_admin_admins[i]->level)
				{
					...
					Com_sprintf(lname_fmt, sizeof(lname_fmt), "%%%is",
					    (admin_level_maxname + strlen(g_admin_levels[j]->name) - strlen(
					        lname)));
					Com_sprintf(lname, sizeof(lname), lname_fmt, g_admin_levels[j]->name);
					break;
				}
...
G_admin_duration(int secs, char *duration, int dursize)
{
	...
	if (secs > (60 * 60 * 24 * 365 * 50) || secs < 0) Q_strncpyz(duration,
	    "PERMANENT", dursize);
	else if (secs >= (60 * 60 * 24 * 365)) Com_sprintf(duration, dursize,
	    "%1.1f years", (secs / (60 * 60 * 24 * 365.0f)));
	else if (secs >= (60 * 60 * 24 * 90)) Com_sprintf(duration, dursize,
	    "%1.1f weeks", (secs / (60 * 60 * 24 * 7.0f)));
	else if (secs >= (60 * 60 * 24)) Com_sprintf(duration, dursize, "%1.1f days",
	    (secs / (60 * 60 * 24.0f)));
	else if (secs >= (60 * 60)) Com_sprintf(duration, dursize, "%1.1f hours",
	    (secs / (60 * 60.0f)));
	else if (secs >= 60) Com_sprintf(duration, dursize, "%1.1f minutes", (secs
	    / 60.0f));
	else Com_sprintf(duration, dursize, "%i seconds", secs);
}
	...
	*reason = '\0';
	...
	for (i = 0; i < MAX_ADMIN_BANS && g_admin_bans[i]; i++)
	{
		...
		if (!ignoreIP)
		{
			...
			memset(IP, 0, sizeof(IP));
			ipscanfcount = sscanf(g_admin_bans[i]->ip, "%d.%d.%d.%d/%d", &IP[4],
			    &IP[3], &IP[2], &IP[1], &IP[0]);

			if (ipscanfcount == 4) mask = -1;
			else if (ipscanfcount == 5) mask = IP[0];
			else if (ipscanfcount > 0 && ipscanfcount < 4) mask = 8 * ipscanfcount;
			else continue;
			...
			{
				char duration[32];
				G_admin_duration((g_admin_bans[i]->expires - t), duration,
				    sizeof(duration));
				...
				if (t - lastConnectTime >= 300 || Q_stricmp(lastConnectIP, ip))
				{
					...
					G_AdminsPrintf(
					    ...
					    Info_ValueForKey(userinfo, "name"), g_admin_bans[i]->name, i + 1,
					    ip, g_admin_bans[i]->banner, duration, g_admin_bans[i]->reason);
				}

				Com_sprintf(reason, rlen,
				    "You have been banned by %s^7 reason: %s^7 expires: %s       %s",
				    g_admin_bans[i]->banner, g_admin_bans[i]->reason, duration, notice);
...
G_admin_cmd_check(gentity_t *ent, qboolean say)
{
	...
	if (!Q_stricmp(command, "say") || (G_admin_permission(ent, ADMF_TEAMCHAT_CMD)
	    && (!Q_stricmp(command, "say_team"))))
	{
		skip = 1;
		G_SayArgv(1, command, sizeof(command));
	}
	if (!command[0]) return qfalse;
	...
	if (g_floodMinTime.integer) if (G_Flood_Limited(ent))
	{
	...
	for (i = 0; i < MAX_ADMIN_COMMANDS && g_admin_commands[i]; i++)
	{
		if (Q_stricmp(cmd, g_admin_commands[i]->command)) continue;
	...
	{
		if (Q_stricmp(cmd, g_admin_cmds[i].keyword)) continue;
...
G_admin_namelog_update(gclient_t *client, qboolean disconnect)
{
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++)
	{
		if (disconnect && g_admin_namelog[i]->slot != clientNum) continue;
		...
		if (!Q_stricmp(client->pers.ip, g_admin_namelog[i]->ip) && !Q_stricmp(
		    client->pers.guid, g_admin_namelog[i]->guid))
		{
			for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES && g_admin_namelog[i]->name[j][0]; j++)
			{
				G_SanitiseString(g_admin_namelog[i]->name[j], n2, sizeof(n2));
				if (!Q_stricmp(n1, n2)) break;
			}
			if (j == MAX_ADMIN_NAMELOG_NAMES) j = MAX_ADMIN_NAMELOG_NAMES - 1;
			...
			if (!disconnect) g_admin_namelog[i]->banned = qfalse;
...
G_admin_readconfig(gentity_t *ent, int skiparg)
{
	...
	*(cnf + len) = '\0';
	...
	while (*t)
	{
		if (!Q_stricmp(t, "[level]") || !Q_stricmp(t, "[admin]") || !Q_stricmp(t,
		    "[ban]") || !Q_stricmp(t, "[command]"))
		{

			if (level_open) g_admin_levels[lc++] = l;
			else if (admin_open) g_admin_admins[ac++] = a;
			else if (ban_open) g_admin_bans[bc++] = b;
			else if (command_open) g_admin_commands[cc++] = c;
			level_open = admin_open = ban_open = command_open = qfalse;
		}
		...
		if (!Q_stricmp(t, "[level]"))
		{
			if (lc >= MAX_ADMIN_LEVELS) return qfalse;
			l = G_Alloc(sizeof(g_admin_level_t));
			l->level = 0;
			*l->name = '\0';
			*l->flags = '\0';
		...
		{
			if (ac >= MAX_ADMIN_ADMINS) return qfalse;
		...
		{
			if (bc >= MAX_ADMIN_BANS) return qfalse;
		...
		{
			if (cc >= MAX_ADMIN_COMMANDS) return qfalse;
	...
	if (admin_open) g_admin_admins[ac++] = a;
	if (ban_open) g_admin_bans[bc++] = b;
	if (command_open) g_admin_commands[cc++] = c;
	G_Free(cnf2);
	ADMP(va(
	    "^3!readconfig: ^7loaded %d levels, %d admins, %d bans, %d commands\n",
	    lc, ac, bc, cc));
	if (lc == 0) admin_default_levels();
	else
	{
		...
		for (i = 0; i < MAX_ADMIN_LEVELS && g_admin_levels[i]; i++)
		{
			...
			if (strlen(n) > admin_level_maxname) admin_level_maxname = strlen(n);
		}
	}
	// reset adminLevel
	for (i = 0; i < level.maxclients; i++)
		if (level.clients[i].pers.connected != CON_DISCONNECTED) level.clients[i].pers.adminLevel
...
G_admin_setlevel(gentity_t *ent, int skiparg)
{
	...
	if (!ent && !g_admin_levels[0]) G_admin_readconfig(NULL, 0);
	...
	if (numeric && id >= 0 && id < level.maxclients) vic = &g_entities[id];
	...
	else
	{
		...
		if (vic) Q_strncpyz(adminname, vic->client->pers.netname, sizeof(adminname));
	}
...
G_admin_parse_time(const char *time)
{
	...
	for (i = 0; time[i]; i++)
	{
		...
		if (i == 0 || !isdigit(time[i - 1])) return -1;
		switch (time[i])
		{
			case 'w':
				num *= 7;
...
static qboolean
admin_create_ban(gentity_t *ent, char *netname, char *guid, char *ip,
    int seconds, char *reason)
{
	g_admin_ban_t *b = NULL;
	qtime_t qt;
	int t;
	...
	Q_strncpyz(b->made, va("%02i/%02i/%02i %02i:%02i:%02i", (qt.tm_mon + 1),
	    qt.tm_mday, (qt.tm_year - 100), qt.tm_hour, qt.tm_min, qt.tm_sec),
	    sizeof(b->made));

	Q_strncpyz(b->banner, G_admin_get_adminname(ent), sizeof(b->banner));

	if (!seconds) b->expires = 0;
	else b->expires = t + seconds;
	if (!*reason) Q_strncpyz(b->reason, "banned by admin", sizeof(b->reason));
	else Q_strncpyz(b->reason, reason, sizeof(b->reason));
	for (i = 0; i < MAX_ADMIN_BANS && g_admin_bans[i]; i++)
		;
	if (i == MAX_ADMIN_BANS)
	{
...
G_admin_kick(gentity_t *ent, int skiparg)
{
	...
	minargc = 3 + skiparg;
	if (G_admin_permission(ent, ADMF_UNACCOUNTABLE)) minargc = 2 + skiparg;
	...
	admin_create_ban(ent, vic->client->pers.netname, vic->client->pers.guid,
	    vic->client->pers.ip, G_admin_parse_time(g_adminTempBan.string),
	    (*reason) ? reason : "kicked by admin");
	if (g_admin.string[0]) admin_writeconfig();

	trap_SendServerCommand(pids[0], va(
	    "disconnect \"You have been kicked.\n%s^7\nreason:\n%s\n%s\"",
	    (ent) ? va("admin:\n%s", G_admin_adminPrintName(ent)) : "admin\nconsole",
	    (*reason) ? reason : "kicked by admin", notice));

	trap_DropClient(pids[0], va("kicked%s^7, reason: %s", (ent) ? va(" by %s",
	    G_admin_adminPrintName(ent)) : " by console", (*reason) ? reason
	    : "kicked by admin"));
...
G_admin_ban(gentity_t *ent, int skiparg)
{
	...
	if (G_admin_permission(ent, ADMF_CAN_PERM_BAN) && G_admin_permission(ent,
	    ADMF_UNACCOUNTABLE))
	{
		minargc = 2 + skiparg;
	}
	...
	else
	{
		...
		if (g_adminMaxBan.integer && seconds > G_admin_parse_time(
		    g_adminMaxBan.string) && !G_admin_permission(ent, ADMF_CAN_PERM_BAN))
		{
			seconds = G_admin_parse_time(g_adminMaxBan.string);
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++)
	{
		...
		if (g_admin_namelog[i]->banned) continue;

		// skip disconnected players when banning on slot number
		if (g_admin_namelog[i]->slot == -1) continue;
	...
	else if (logmatches > 1)
	{
		...
		for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++)
		{
			...
			for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES && g_admin_namelog[i]->name[j][0]; j++)
			{
				...
				if (strstr(n2, s2))
				{
					if (g_admin_namelog[i]->slot > -1) ADMBP("^3");
	...
	admin_create_ban(ent, g_admin_namelog[logmatch]->name[0],
	    g_admin_namelog[logmatch]->guid, g_admin_namelog[logmatch]->ip, seconds,
	    reason);
	...
	trap_SendServerCommand(g_admin_namelog[logmatch]->slot, va(
	    "disconnect \"You have been banned.\n"
		    "admin:\n%s^7\nduration:\n%s\nreason:\n%s\n%s\"",
	    (ent) ? G_admin_adminPrintName(ent) : "console", duration,
	    (*reason) ? reason : "banned by admin", notice));

	trap_DropClient(g_admin_namelog[logmatch]->slot, va(
	    "banned by %s^7, duration: %s, reason: %s",
	    (ent) ? G_admin_adminPrintName(ent) : "console", duration,
	    (*reason) ? reason : "banned by admin"));
	return qtrue;
}
...
G_admin_adjustban(gentity_t *ent, int skiparg)
{
	...
	if (g_admin_bans[bnum - 1]->expires == 0 && !G_admin_permission(ent,
	    ADMF_CAN_PERM_BAN))
	{
	...
	if (g_adminMaxBan.integer && !G_admin_permission(ent, ADMF_CAN_PERM_BAN)
	    && g_admin_bans[bnum - 1]->expires - time > G_admin_parse_time(
	        g_adminMaxBan.string))
	{
		ADMP(va(
	...
	else
	{
		if (length)
		{
			...
			if (mode == '+') expires = g_admin_bans[bnum - 1]->expires + length;
			else if (mode == '-') expires = g_admin_bans[bnum - 1]->expires - length;
			else expires = time + length;
			if (expires <= time)
			{
		...
		else if (G_admin_permission(ent, ADMF_CAN_PERM_BAN)) expires = 0;
		else
		{
			ADMP("^3!adjustban: ^7ban time must be positive\n");
			return qfalse;
	...
	reason = G_SayConcatArgs(3 + skiparg);
	if (*reason) Q_strncpyz(g_admin_bans[bnum - 1]->reason, reason,
	...
	AP(
	    va("print \"^3!adjustban: ^7ban #%d for %s^7 has been updated by %s^7 "
	        ...
	        reason));
	if (ent) Q_strncpyz(g_admin_bans[bnum - 1]->banner,
	    G_admin_get_adminname(ent), sizeof(g_admin_bans[bnum - 1]->banner));
	if (g_admin.string[0]) admin_writeconfig();
	return qtrue;
}
...
G_admin_subnetban(gentity_t *ent, int skiparg)
{
	...
	if (mask >= 0 && mask <= 32)
	{
		...
		sscanf(g_admin_bans[bnum - 1]->ip, "%d.%d.%d.%d/%d", &IP[4], &IP[3],
		    &IP[2], &IP[1], &IP[0]);
		for (k = 4; k >= 1; k--)
		{
			if (!IP[k]) IP[k] = 0;
		...
		if (mask == 32)
		{
			Q_strncpyz(g_admin_bans[bnum - 1]->ip, va("%i.%i.%i.%i", IP[4], IP[3],
			    IP[2], IP[1]), sizeof(g_admin_bans[bnum - 1]->ip));
		}
		else
		{
			Q_strncpyz(g_admin_bans[bnum - 1]->ip, va("%i.%i.%i.%i/%i", IP[4], IP[3],
			    IP[2], IP[1], mask), sizeof(g_admin_bans[bnum - 1]->ip));
			IPRlow &= ~((1 << (32 - mask)) - 1);
			IPRhigh |= ((1 << (32 - mask)) - 1);
		}
	}
	...
	{
		Q_strncpyz(cIPRlow,
		    va("%hhu.%hhu.%hhu.%hhu", (IPRlow & (255 << 24)) >> 24, (IPRlow & (255
		        << 16)) >> 16, (IPRlow & (255 << 8)) >> 8, IPRlow & 255),
		    sizeof(cIPRlow));
		Q_strncpyz(cIPRhigh, va("%hhu.%hhu.%hhu.%hhu", (IPRhigh & (255 << 24))
		    >> 24, (IPRhigh & (255 << 16)) >> 16, (IPRhigh & (255 << 8)) >> 8,
		    IPRhigh & 255), sizeof(cIPRhigh));
	}
	else
	{
...
G_admin_unban(gentity_t *ent, int skiparg)
{
	...
	if (g_adminMaxBan.integer && !G_admin_permission(ent, ADMF_CAN_PERM_BAN)
	    && g_admin_bans[bnum - 1]->expires - t > G_admin_parse_time(
	        g_adminMaxBan.string))
	{
...
G_admin_putteam(gentity_t *ent, int skiparg)
{
	...
	if (vic->client->pers.teamSelection == teamnum) return qfalse;
...
G_admin_map(gentity_t *ent, int skiparg)
{
	...
	if (G_SayArgc() > 2 + skiparg)
	{
		...
		if (!Q_stricmp(layout, "*BUILTIN*") || trap_FS_FOpenFile(va(
		    "layouts/%s/%s.dat", map, layout), NULL, FS_READ) > 0)
		{
...
G_admin_maplog_update(void)
{
	...
	trap_Cvar_Set("g_adminMapLog", va("%s%s%s", map, (maplog[0] != '\0') ? " "
	    : "", maplog));
}
...
G_admin_maplog_result(char *flag)
{
	...
	if (level.surrenderTeam != PTE_NONE)
	{
		if (flag[0] == 'a')
		{
			if (level.surrenderTeam == PTE_HUMANS) flag = "A";
		}
		else if (flag[0] == 'h')
		{
			if (level.surrenderTeam == PTE_ALIENS) flag = "H";
		}
	}

	t = (level.time - level.startTime) / 1000;
	Q_strncpyz(maplog, g_adminMapLog.string, sizeof(maplog));
	trap_Cvar_Set("g_adminMapLog", va("%1s;%03d:%02d;%s", flag, t / 60, t % 60,
	    maplog));
}
...
G_admin_listadmins(gentity_t *ent, int skiparg)
{
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++)
	{
		if (g_admin_admins[i]->level >= minlevel) found++;
	}
	if (!found)
	{
		if (minlevel > 1)
	...
	if (G_SayArgc() >= 2 + skiparg)
	{
		...
		if (numeric)
		{
			start = atoi(s);
			if (start > 0) start -= 1;
			else if (start < 0) start = found + start;
		}
		else G_SanitiseString(s, search, sizeof(search));
	}

	if (start >= found || start < 0) start = 0;
...
G_admin_listlayouts(gentity_t *ent, int skiparg)
{
	...
	if (G_SayArgc() == 2 + skiparg) G_SayArgv(1 + skiparg, map, sizeof(map));
	else trap_Cvar_VariableStringBuffer("mapname", map, sizeof(map));
...
G_admin_listmaps(gentity_t *ent, int skiparg)
{
	...
	filePtr = fileList;
	for (i = 0; i < numFiles && count < MAX_LISTMAPS_MAPS; i++, filePtr
	    += fileLen + 1)
	{
		...
		if (search[0] && !strstr(filePtr, search)) continue;
	...
	if (search[0]) ADMBP(va("^3!listmaps: ^7found %d maps matching '%s^7'.\n",
	    count, search));
	else ADMBP(va("^3!listmaps: ^7listing %d maps.\n", count));
...
G_admin_showbans(gentity_t *ent, int skiparg)
{
	...
	if (start >= MAX_ADMIN_BANS || start < 0) start = 0;

	for (i = start; i < MAX_ADMIN_BANS && g_admin_bans[i] && show_count
	    < MAX_ADMIN_SHOWBANS; i++)
	{
		...
		if (!numeric)
		{
			if (!subnetfilter)
			{
				...
				if (strstr(n1, name_match)) match = qtrue;
			}
			else
			{
				int mask = -1;
				int dummy;
				int scanflen = 0;
				scanflen = sscanf(g_admin_bans[i]->ip, "%d.%d.%d.%d/%d", &dummy,
				    &dummy, &dummy, &dummy, &mask);
		...
		if ((match) || !ip_match || Q_strncmp(ip_match, g_admin_bans[i]->ip,
		    ip_match_len) == 0)
		{
			...
			if (strlen(n2) > max_banner) max_banner = strlen(n2);
	    ...
	    < MAX_ADMIN_SHOWBANS; i++)
	{
		...
		if (!numeric)
		{
			...
			{
				G_SanitiseString(g_admin_bans[i]->name, n1, sizeof(n1));
				if (strstr(n1, name_match) == NULL) continue;
			}
			else
			{
				int mask = -1;
		...
		else if (ip_match != NULL && Q_strncmp(ip_match, g_admin_bans[i]->ip,
		    ip_match_len) != 0) continue;
		...
		G_DecolorString(g_admin_bans[i]->name, n1);
		Com_sprintf(name_fmt, sizeof(name_fmt), "%%%is", (max_name + strlen(
		    g_admin_bans[i]->name) - strlen(n1)));
		Com_sprintf(n1, sizeof(n1), name_fmt, g_admin_bans[i]->name);

		G_DecolorString(g_admin_bans[i]->banner, n2);
		Com_sprintf(banner_fmt, sizeof(banner_fmt), "%%%is", (max_banner + strlen(
		    g_admin_bans[i]->banner) - strlen(n2)));
	...
	{
		char matchmethod[50];
		if (numeric) Com_sprintf(matchmethod, sizeof(matchmethod), "IP");
		else if (!subnetfilter) Com_sprintf(matchmethod, sizeof(matchmethod),
		    "name");
		else Com_sprintf(matchmethod, sizeof(matchmethod), "ip range size");
...
G_admin_help(gentity_t *ent, int skiparg)
{
	...
	if (G_SayArgc() < 2 + skiparg)
	{
		...
		for (i = 0; i < MAX_ADMIN_COMMANDS && g_admin_commands[i]; i++)
		{
			if (!G_admin_permission(ent, g_admin_commands[i]->flag)) continue;
		...
		if (ent) strcat(additional, " /builder /say_area");
		if (g_publicSayadmins.integer || G_admin_permission(ent, ADMF_ADMINCHAT)) strcat(
		    additional, " /a /say_admins");
		if (g_privateMessages.integer) strcat(additional, " /m");
		if (ent && g_actionPrefix.string[0]) strcat(additional, " /me /mt /me_team");
		if (ent && g_myStats.integer) strcat(additional, " /mystats");
		if (ent && ent->client)
		{
		...
		if (ent && g_allowShare.integer) strcat(additional, " /share /donate");
...
G_admin_admintest(gentity_t *ent, int skiparg)
{
	...
	if (found)
	{
		...
		for (i = 0; i < MAX_ADMIN_LEVELS && g_admin_levels[i]; i++)
		{
			if (g_admin_levels[i]->level != l) continue;
...
G_admin_allready(gentity_t *ent, int skiparg)
{
	...
	for (i = 0; i < g_maxclients.integer; i++)
	{
		...
		if (cl->pers.teamSelection == PTE_NONE) continue;
...
G_admin_printgrid(gentity_t *ent, int skiparg)
{
	...
	for (i = MAX_CLIENTS; i < level.num_entities; i++)
	{
		...
		if (node->s.eType != ET_BUILDABLE) continue;
		if (node->biteam != BIT_HUMANS) continue;
		if (node->s.modelindex != BA_H_SPAWN) continue;
...
G_switchnodes(gentity_t *ent, int skiparg)
{
	...
	for (i = MAX_CLIENTS; i < level.num_entities; i++)
	{
		...
		if (ent2->health <= 0) continue;
		if (ent2->s.eType != ET_BUILDABLE) continue;
		if (ent2->biteam != BIT_ALIENS) continue;
		if (ent2->s.modelindex != BA_H_SPAWN) continue;
		...
		for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++)
		{
			if (g_admin_namelog[i]->slot >= 0)
			{
				    ...
				    && g_admin_namelog[i]->name[j][0]; j++)
				{
					...
					G_SanitiseString(g_admin_namelog[i]->name[j], name2_s,
					    sizeof(name2_s));
...
G_admin_spec999(gentity_t *ent, int skiparg)
{
	...
	for (i = 0; i < level.maxclients; i++)
	{
		...
		if (vic->client->pers.teamSelection == PTE_NONE) continue;
...
G_admin_register(gentity_t *ent, int skiparg)
{
	...
	if (g_newbieNumbering.integer && g_newbieNamePrefix.string[0] && !Q_stricmpn(
	    ent->client->pers.netname, g_newbieNamePrefix.string, strlen(
...
G_admin_rename(gentity_t *ent, int skiparg)
{
	...
	ClientUserinfoChanged(pids[0]);
	if (strcmp(oldname, level.clients[pids[0]].pers.netname)) AP(va(
...
G_admin_restart(gentity_t *ent, int skiparg)
{
	...
	if (G_SayArgc() > 1 + skiparg)
	{
		...
		if (Q_stricmp(layout, "keepteams") && Q_stricmp(layout, "keepteamslock")
		    && Q_stricmp(layout, "switchteams") && Q_stricmp(layout,
		    "switchteamslock"))
		{
			if (!Q_stricmp(layout, "*BUILTIN*") || trap_FS_FOpenFile(va(
			    "layouts/%s/%s.dat", map, layout), NULL, FS_READ) > 0)
	...
	else if (!Q_stricmp(teampref, "switchteams") || !Q_stricmp(teampref,
	    "switchteamslock"))
	{
		for (i = 0; i < g_maxclients.integer; i++)
		{
			...
			if (cl->pers.teamSelection == PTE_ALIENS) cl->sess.restartTeam
			    = PTE_HUMANS;
			else if (cl->pers.teamSelection == PTE_HUMANS) cl->sess.restartTeam
			    = PTE_ALIENS;
		}
	}

	if (!Q_stricmp(teampref, "switchteamslock") || !Q_stricmp(teampref,
	    "keepteamslock"))
	{
	...
	if (teampref[0]) strcpy(teampref, va("^7(with teams option: '%s^7')",
	    teampref));
...
G_admin_nextmap(gentity_t *ent, int skiparg)
{
	...
	LogExit(va("nextmap was run by %s", (ent) ? G_admin_adminPrintName(ent)
	    : "console"));
...
G_admin_lock(gentity_t *ent, int skiparg)
{
	...
	if (teamName[0] == 'a' || teamName[0] == 'A') team = PTE_ALIENS;
	else if (teamName[0] == 'h' || teamName[0] == 'H') team = PTE_HUMANS;
	else
	{
	...
	if (team == PTE_ALIENS)
	{
		...
		else level.alienTeamLocked = qtrue;
	}
	else if (team == PTE_HUMANS)
	{
		...
		else level.humanTeamLocked = qtrue;
	}
...
G_admin_unlock(gentity_t *ent, int skiparg)
{
	...
	if (team == PTE_ALIENS)
	{
		...
		else level.alienTeamLocked = qfalse;
	}
	else if (team == PTE_HUMANS)
	{
		if (!level.humanTeamLocked)
		...
		else level.humanTeamLocked = qfalse;
	}
...
G_admin_putmespec(gentity_t *ent, int skiparg)
{
	...
	if (ent->client->pers.teamSelection == PTE_NONE) return qfalse;
...
#define LOG_DISPLAY_LENGTH 10
	...
	if (skip > len - LOG_DISPLAY_LENGTH) skip = len - LOG_DISPLAY_LENGTH;
	*message = '\0';
	// skip to start entry
	for (ptr = level.buildHistory, i = len; ptr && i > len - skip; ptr
	    = ptr->next)
	{
		// these checks could perhaps be done more efficiently but they are cheap
		// in processor time so I'm not worrying
		if (team != PTE_NONE && team != BG_FindTeamForBuildable(ptr->buildable)) continue;
		if (builder && builder != ptr->ent) continue;
		matchlen++;
		i--;
	}
	for (; i + LOG_DISPLAY_LENGTH > len - skip && i > 0; i--, ptr = ptr->next)
	{
		if (!ptr) break; // run out of log
		*markstring = '\0'; // reinit markstring
		...
		if (lastID < 0) lastID = ptr->ID;
		firstID = ptr->ID;
		matchlen++;
		// set name to the ent's current name or last recorded name
		if (ptr->ent)
		{
			if (ptr->ent->client) name = ptr->ent->client->pers.netname;
			else name = "<world>"; // any non-client action
		}
		else name = ptr->name;
		switch (ptr->fate)
		{
		...
		if (ptr->marked)
		{
			...
			for (j = BA_NUM_BUILDABLES; j >= 0; j--)
			{
				...
				// plural is easy
				if (markdecon[j] > 1) Com_sprintf(bnames, 32, "%d %ss", markdecon[j],
				    buildablename);
				// use an appropriate article
				else if (markdecon[j] == 1)
				{
					if (BG_FindUniqueTestForBuildable(j)) article = "the"; // if only one
					else if (strchr("aeiouAEIOU", *buildablename)) article = "an"; // if first char is vowel
					else article = "a";
					Com_sprintf(bnames, 32, "%s %s", article, buildablename);
				}
				else continue; // none of this buildable
				// C grammar: x, y, and z
		...
		Com_sprintf(message, MAX_STRING_CHARS, "%3d %s^7 %s^7 %s%s %s%s%s\n%s",
		    ...
		    (strchr("aeiouAEIOU", buildablename[0])) ? "n" : "", buildablename,
		    (markstring[0]) ? ", removing " : "", markstring, message);
	}
	...
	if (matchlen) ADMP(va("%s^3!buildlog: showing log entries %d - %d of %d\n",
	    message, firstID, lastID, matchlen));
	else ADMP("^3!buildlog: ^7no log entries match those criteria\n");
	return qtrue;
}
...
G_admin_revert(gentity_t *ent, int skiparg)
{
	...
	{
		if (!ptr) break; // run out of bhist
		if (!reached && ID)
		{
			if (ptr->ID == ID) reached = qtrue;
			else
			{
				prev = ptr;
				ptr = ptr->next;
		...
		{
			if (ptr->ent->client) name = ptr->ent->client->pers.netname;
			else name = "<world>"; // non-client actions
		}
		else name = ptr->name;
		...
		switch (ptr->fate)
		{
			case BF_BUILT:
				...
				for (j = MAX_CLIENTS, targ = g_entities + j; j < level.num_entities; j++, targ++)
				{
					...
					if (targ->s.eType != ET_BUILDABLE) continue;
					if (targ->s.modelindex != ptr->buildable) continue;
					VectorSubtract(targ->s.pos.trBase, ptr->origin, dist);
#define FIND_BUILDABLE_TOLERANCE 5
					if (VectorLength(dist) > FIND_BUILDABLE_TOLERANCE) continue; // number is somewhat arbitrary, watch for false pos/neg
						...
						if (!G_RevertCanFit(mark))
						{
							...
							Com_sprintf(argbuf, sizeof argbuf, "%s%s%s%s%s%s%s!",
							    (repeat > 1) ? "x" : "", (repeat > 1) ? va("%d ", repeat)
							        : "", (ID) ? "#" : "", (ID) ? va("%d ", ptr->ID) : "",
							    (builder) ? "-" : "", (builder) ? va("%d ", builder
							        - g_entities) : "", (team == PTE_ALIENS) ? "a " : (team
							        == PTE_HUMANS) ? "h " : "");
							ADMP(va(
			...
			case BF_DESTROYED:
				...
				if (!force && !G_RevertCanFit(ptr))
				{
					Com_sprintf(argbuf, sizeof argbuf, "%s%s%s%s%s%s%s!",
					    (repeat > 1) ? "x" : "", (repeat > 1) ? va("%d ", repeat) : "",
					    (ID) ? "#" : "", (ID) ? va("%d ", ptr->ID) : "", (builder) ? "-"
					        : "", (builder) ? va("%d ", builder - g_entities) : "", (team
					        == PTE_ALIENS) ? "a " : (team == PTE_HUMANS) ? "h " : "");
					ADMP(
					    va(
		...
		if (BG_FindUniqueTestForBuildable(ptr->buildable)) article = "the";
		else if (strchr("aeiouAEIOU", *bname)) article = "an";
		else article = "a";
		...
		tmp = ptr;
		if (ptr == level.buildHistory) prev = level.buildHistory = ptr = ptr->next;
		else prev->next = ptr = ptr->next;
		G_Free(tmp);
	}
	if (ID && !reached)
	{
...
G_admin_info(gentity_t *ent, int skiparg)
{
	...
	if (G_SayArgc() == 2 + skiparg) G_SayArgv(1 + skiparg, filename,
	    sizeof(filename));
	else if (G_SayArgc() == 1 + skiparg) Q_strncpyz(filename, "default",
	    sizeof(filename));
	else
	{
	...
	else
	{
		...
		if (length < sizeof(message)) message[length] = '\0';
		else message[sizeof(message) - 1] = '\0';
		trap_FS_FCloseFile(infoFile);
...
#define MAX_INFO_PARSE_LOOPS 100
		for (i = 0; i < MAX_INFO_PARSE_LOOPS && G_StringReplaceCvars(message,
		    message, sizeof(message)); i++)
			;
...
G_StringReplaceCvars(char *input, char *output, int len)
{
	...
	while (*input && outNum < len)
	{
		...
		else outputBuffer[outNum++] = *input++;
	}
...
{
	if (ent) trap_SendServerCommand(ent - level.gentities, va("print \"%s\"", m));
	else
	{
