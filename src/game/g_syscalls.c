...
{
  syscall( G_SQL_FINISHQUERY );
}

void trap_mysql_cleanstring( const char *in, char *out, int len )
{
  syscall( G_SQL_CLEANSTRING, in, out, len );
}

qboolean trap_mysql_fetchrow( void )
{
  return syscall( G_SQL_FETCHROW );
}

int trap_mysql_rowcount( void )
{
  return syscall( G_SQL_ROWCOUNT );
}

int trap_mysql_affectedrows( void )
{
  return syscall( G_SQL_AFFECTEDROWS );
}
...
int trap_mysql_fieldcount( void )
{
  syscall( G_SQL_FIELDCOUNT );
}
