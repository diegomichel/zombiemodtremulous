 ...
 */
static int
G_NumberOfDependants(gentity_t *self)
{
  int i, n = 0;
  gentity_t *ent;
...
static void
G_DestroyAlienSpawns(gentity_t *self, int range)
{
  int i;
  gentity_t *ent;
  int distance;
  ...
  trap_SendServerCommand(-1, va("print \"This functions shuld not be called G_DestroyAlienSpawns\n\""));
...
G_FindPower(gentity_t *self)
{
  ...
  if (self->s.modelindex == BA_H_REPEATER && g_survival.integer && self->biteam == BIT_HUMANS)
    return qtrue;

  //if this already has power then stop now
  if (self->parentNode && self->parentNode->powered)
  {
    //If we have wasted all the money on an armory and its parent is a repeator then kill the parent.
    if (g_survival.integer && self->used && level.survivalmoney >= 500 && self->parentNode->s.modelindex == BA_H_REPEATER && self->biteam == PTE_HUMANS)
    {
  ...
  for(i = 1, ent = g_entities + i;i < level.num_entities;i++, ent++)
  {
    ...
    if ((ent->s.modelindex == BA_H_REACTOR || ent->s.modelindex == BA_H_REPEATER) && ent->spawned && (ent->biteam == biteam))
    {
      VectorSubtract(self->s.origin, ent->s.origin, temp_v);
      distance = VectorLength(temp_v);

      if (distance < minDistance && ent->powered && ((ent->s.modelindex == BA_H_REACTOR && distance <= REACTOR_BASESIZE) || (ent->s.modelindex == BA_H_REPEATER
  ...
  {
    if (g_survival.integer && self->s.modelindex == BA_H_SPAWN && self->biteam == BIT_HUMANS && self->health > 0)
    {
      /*
       if(ent->survivalchance == 0) // Will allow go back to main loop and get a chance to get powered.
       {
       ent->survivalchance = 1;
       return qfalse;
       }
       //level.survivalmoney = 0;
       if ((level.time - level.startTime) > 6000) {//Prevent  the game get fucked when loading map.
       trap_SendServerCommand(-1, "print \"This shuld only happend 1 time per struct..\n\"");
...
G_FindDCC(gentity_t *self)
{
  ...
  for(i = 1, ent = g_entities + i;i < level.num_entities;i++, ent++)
  {
    ...
    if (ent->s.modelindex == BA_H_DCC && ent->spawned)
    {
      VectorSubtract(self->s.origin, ent->s.origin, temp_v);
      distance = VectorLength(temp_v);
      if (distance < minDistance && ent->powered && (self->biteam == ent->biteam))
      {
        closestDCC = ent;
        minDistance = distance;
        foundDCC = qtrue;
...
G_CreepSlow(gentity_t *self)
{
  ...
  for(i = 0;i < num;i++)
  {
    ...
    if (enemy->client && enemy->client->ps.stats[STAT_PTEAM] == PTE_HUMANS && enemy->client->ps.groundEntityNum != ENTITYNUM_NONE && G_Visible(self, enemy))
    {
 ...
 */
static void
nullDieFunction(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
}
...
A_CreepRecede(gentity_t *self)
{
  ...
  if (!(self->s.eFlags & EF_DEAD))
  {
    ...
    if (self->spawned)
      ...
      self->s.time = -(level.time - (int) ((float) CREEP_SCALEDOWN_TIME * (1.0f - ((float) (level.time - self->buildTime)
 ...
 */
void
ASpawn_Melt(gentity_t *self)
{
  G_SelectiveRadiusDamage(self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath, PTE_ALIENS);
 ...
 */
void
ASpawn_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
  buildHistory_t *new;
  ...
  if (attacker && attacker->client)
  {
    ...
    G_LogPrintf("Decon: %i %i %i: %s^7 destroyed %s by %s\n", attacker->client->ps.clientNum, self->s.modelindex, mod, attacker->client->pers.netname,
        BG_FindNameForBuildable(self->s.modelindex), modNames[mod]);
  }
}
...
ASpawn_Think(gentity_t *self)
{
  ...
  if (self->spawned)
  {
    ...
    {
      if ((ent = G_CheckSpawnPoint(self->s.number, self->s.origin, self->s.origin2, BA_A_SPAWN, NULL)) != NULL)
      {
        ...
        else if (g_antiSpawnBlock.integer && ent->client && ent->client->ps.stats[STAT_PTEAM] == PTE_ALIENS)
        {
 ...
 */
void
ASpawn_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
 ...
 */
void
AOvermind_Think(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t range =
  { OVERMIND_ATTACK_RANGE, OVERMIND_ATTACK_RANGE, OVERMIND_ATTACK_RANGE };
  vec3_t mins, maxs;
  int i, num;
  gentity_t *enemy;
  ...
  if (self->spawned && (self->health > 0))
  {
    ...
    for(i = 0;i < num;i++)
    {
      ...
      {
        self->timestamp = level.time;
        G_SelectiveRadiusDamage(self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, MOD_OVERMIND, PTE_ALIENS);
    ...
    if (!level.overmindMuted && level.numAlienSpawns <= 0 && level.time > self->overmindSpawnsTimer)
    {
      ...
      for(i = 0;i < level.numConnectedClients;i++)
      {
        ...
        if (builder->health > 0 && (builder->client->pers.classSelection == PCL_ALIEN_BUILDER0 || builder->client->pers.classSelection
 ...
 */
void
ABarricade_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
  if (rand() % 2)
    G_SetBuildableAnim(self, BANIM_PAIN1, qfalse);
  else
 ...
 */
void
ABarricade_Blast(gentity_t *self)
{
  vec3_t dir;
 ...
 */
void
ABarricade_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
  buildHistory_t *new;
  new = G_Alloc(sizeof(buildHistory_t));
  new->ID = (++level.lastBuildID > 1000) ? (level.lastBuildID = 1) : level.lastBuildID;
 ...
 */
void
ABarricade_Think(gentity_t *self)
{
...
void
AAcidTube_Think(gentity_t *self);
 ...
 */
void
AAcidTube_Damage(gentity_t *self)
{
  if (self->spawned)
  {
...
AAcidTube_Think(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t range =
  { ACIDTUBE_RANGE, ACIDTUBE_RANGE, ACIDTUBE_RANGE };
  vec3_t mins, maxs;
  int i, num;
  gentity_t *enemy;
...
AHovel_Blocked(gentity_t *hovel, gentity_t *player, qboolean provideExit)
{
  ...
  BG_FindBBoxForClass(player->client->ps.stats[STAT_PCLASS], mins, maxs, NULL, NULL, NULL);
 ...
 */
static qboolean
APropHovel_Blocked(vec3_t origin, vec3_t angles, vec3_t normal, gentity_t *player)
{
  gentity_t hovel;
 ...
 */
void
AHovel_Use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
  ...
  if (self->spawned && G_FindOvermind(self))
  {
    ...
    else if (((activator->client->ps.stats[STAT_PCLASS] == PCL_ALIEN_BUILDER0) || (activator->client->ps.stats[STAT_PCLASS] == PCL_ALIEN_BUILDER0_UPG))
 ...
 */
void
AHovel_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
  vec3_t dir;
 ...
 */
void
ABooster_Touch(gentity_t *self, gentity_t *other, trace_t *trace)
{
 ...
 */
void
ATrapper_FireOnEnemy(gentity_t *self, int firespeed, float range)
{
  ...
  int lowMsec = 0;
  int highMsec = (int) ((((distanceToTarget * LOCKBLOB_SPEED) + (distanceToTarget * BG_FindSpeedForClass(enemy->client->ps.stats[STAT_PCLASS])))
 ...
 */
qboolean
ATrapper_CheckTarget(gentity_t *self, gentity_t *target, int range)
{
  vec3_t distance;
  trace_t trace;
 ...
 */
void
ATrapper_FindEnemy(gentity_t *ent, int range)
{
  gentity_t *target;
 ...
 */
void
HRepeater_Think(gentity_t *self)
{
  int i;
  ...
  if (self->spawned)
  {
    if (g_survival.integer)
      reactor = qtrue; //No need rc on survival modes.
    ...
    for(i = 1, ent = g_entities + i;i < level.num_entities;i++, ent++)
    {
      ...
      if (ent->s.modelindex == BA_H_REACTOR && ent->spawned && (ent->biteam == self->biteam))
        reactor = qtrue;
    }
  }
 ...
 */
void
HRepeater_Use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
  if (self->health <= 0)
    return;
 ...
 */
void
HReactor_Think(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t range =
  { REACTOR_ATTACK_RANGE, REACTOR_ATTACK_RANGE, REACTOR_ATTACK_RANGE };
  vec3_t mins, maxs;
  int i, num, team;
  ...
  if (self->spawned && (self->health > 0))
  {
    ...
    for(i = 0;i < num;i++)
    {
      ...
      if (enemy->client && (enemy->client->ps.stats[STAT_PTEAM] != team))
      {
        self->timestamp = level.time;
        G_SelectiveRadiusDamage(self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE, REACTOR_ATTACK_RANGE, self, MOD_REACTOR, team);
    ...
    if (self->health < self->lastHealth && level.time > level.humanBaseAttackTimer && G_IsDCCBuilt(self->biteam))
    {
 ...
 */
void
HArmoury_Activate(gentity_t *self, gentity_t *other, gentity_t *activator)
{
  int team;
  ...
  if (self->spawned)
  {
    ...
    if ((activator->client->ps.stats[STAT_PTEAM] != PTE_HUMANS && activator->client->ps.stats[STAT_PTEAM] != PTE_ALIENS)
        || (activator->client->ps.stats[STAT_PTEAM] != team))
      return;
 ...
 */
void
HMedistat_Think(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t mins, maxs;
  int i, num;
  ...
  if (self->spawned)
  {
    ...
    for(i = 0;i < num;i++)
    {
      ...
      if (player->client && (player->client->ps.stats[STAT_PTEAM] == team))
      {
        if (player->health < player->client->ps.stats[STAT_MAX_HEALTH] && player->client->ps.pm_type != PM_DEAD && self->enemy == player)
          occupied = qtrue;
      }
    }
    ...
    if (!occupied)
    {
      ...
      for(i = 0;i < num;i++)
      {
        ...
        if (player->client && (player->client->ps.stats[STAT_PTEAM] == team))
        {
          if (player->health < player->client->ps.stats[STAT_MAX_HEALTH] && player->client->ps.pm_type != PM_DEAD)
          {
            self->enemy = player;
    ...
    else if (self->enemy) //heal!
    {
      ...
      if (self->enemy->health >= self->enemy->client->ps.stats[STAT_MAX_HEALTH] && !BG_InventoryContainsUpgrade(UP_MEDKIT, self->enemy->client->ps.stats))
 ...
 */
qboolean
HMGTurret_TrackEnemy(gentity_t *self)
{
  ...
  if (abs(angleToTarget[YAW] - self->s.angles2[YAW]) <= accuracyTolerance && abs(angleToTarget[PITCH] - self->s.angles2[PITCH]) <= accuracyTolerance)
    return qtrue;
 ...
 */
qboolean
HMGTurret_CheckTarget(gentity_t *self, gentity_t *target, qboolean ignorePainted)
{
  trace_t trace;
  gentity_t *traceEnt;
  int team;
  ...
  {
    if (g_ctn.integer > 0 && target->s.modelindex == BA_H_SPAWN)
    {
      return qfalse;
    }
    if (self->biteam == target->biteam)
    {
      return qfalse;
    }
  }
  ...
  if ((traceEnt->s.eType == ET_BUILDABLE) && (traceEnt->biteam == self->biteam))
    return qfalse;

  if (traceEnt->client && (traceEnt->client->ps.stats[STAT_PTEAM] == team))
    return qfalse;
 ...
 */
void
HMGTurret_FindEnemy(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t range;
  vec3_t mins, maxs;
  ...
  for(i = 0;i < num;i++)
  {
    ...
    if ((target->biteam != self->biteam) || (
        target->client && (target->client->ps.stats[STAT_PTEAM] != team)))
    {
      //if target is not valid keep searching
  ...
  if (self->dcced)
  {
    ...
    for(i = 0;i < num;i++)
    {
      ...
      if (target->client && (target->client->ps.stats[STAT_PTEAM] != team))
      {
        //if target is not valid keep searching
 ...
 */
void
HMGTurret_Think(gentity_t *self)
{
  int firespeed = BG_FindFireSpeedForBuildable(self->s.modelindex);
 ...
 */
void
HTeslaGen_Think(gentity_t *self)
{
  int entityList[MAX_GENTITIES];
  vec3_t range;
  vec3_t mins, maxs;
  ...
  if (self->spawned && self->count < level.time)
  {
    ...
    for(i = 0;i < num;i++)
    {
      ...
      {
        if (g_ctn.integer > 0 && enemy->s.modelindex == BA_H_SPAWN)
        {
          continue;
        }
        if (self->biteam == enemy->biteam)
        {
          continue;
        }
      }
      ...
      if (enemy->client->ps.stats[STAT_PTEAM] == PTE_HUMANS)
      {
        biteam = BIT_HUMANS;
      }
      else
      ...
      /* if( enemy->client &&
       ( enemy->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS || enemy->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS  ) &&
       */if (enemy->health > 0 && Distance(enemy->s.pos.trBase, self->s.pos.trBase) <= TESLAGEN_RANGE)
      {
 ...
 */
void
HSpawn_Disappear(gentity_t *self)
{
  vec3_t dir;
...
HSpawn_Blast(gentity_t *self)
{
  ...
  G_RadiusDamage(self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath);
 ...
 */
void
HSpawn_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
  buildHistory_t *new;
  int team;
  if (self->biteam == BIT_ALIENS)
  ...
  VectorCopy(self->s.origin2, new->origin2);
  VectorCopy(self->s.angles2, new->angles2);
  new->fate = (attacker && attacker->client && (attacker->client->ps.stats[STAT_PTEAM] == team)) ? BF_TEAMKILLED : BF_DESTROYED;
  new->next = NULL;
  G_LogBuild(new);
  ...
  if (attacker && attacker->client)
  {
    if (attacker->client->ps.stats[STAT_PTEAM] != team)
    {
      if (self->s.modelindex == BA_H_REACTOR)
...
HSpawn_Think(gentity_t *self)
{
  ...
  {
    //only suicide if at rest
    if (self->s.groundEntityNum && level.time % 1000 == 0) // Check spawnpoint is expensive
    {
      if (
          (ent = G_CheckSpawnPoint(self->s.number, self->s.origin, self->s.origin2, BA_H_SPAWN, NULL)) != NULL)
      {
        // If the thing blocking the spawn is a buildable, kill it. 
        // If it's part of the map, kill self. 
        if (ent->s.eType == ET_BUILDABLE)
        ...
        else if (g_ctn.integer > 0 && ent->client && ent->health > 0 && self->biteam != ent->client->ps.stats[STAT_PTEAM])
        {
          if (level.time - self->antispawncamp > g_ctncapturetime.integer)
          {
            trap_SendServerCommand(-1, va("print \"^7%s^2 captured a telenode!\n\"", ent->client->pers.netname));
            self->spawnBlockTime = level.time;
            self->s.modelindex2 = self->biteam = ent->client->ps.stats[STAT_PTEAM];
            self->antispawncamp = level.time;
            G_AddCreditToClient(ent->client, 150, qtrue);
          }
        }
        else if (g_antiSpawnBlock.integer && ent->client) // && //ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS ) it push everything out
        {
          //spawnblock protection
          if (self->spawnBlockTime && level.time - self->spawnBlockTime > 10000)
          {
 ...
 */
void
G_BuildableTouchTriggers(gentity_t *ent)
{
  int i, num;
...
G_BuildableThink(gentity_t *ent, int msec)
{
  ...
  if (ent->time1000 >= 1000)
  {
    ...
    else if (ent->biteam == BIT_ALIENS && ent->health > 0 && ent->health < bHealth && bRegen && (ent->lastDamageTime + ALIEN_REGEN_DAMAGE_TIME) < level.time)
      ent->health += bRegen;
...
static qboolean
G_BoundsIntersect(const vec3_t mins, const vec3_t maxs, const vec3_t mins2, const vec3_t maxs2)
{
 ...
 */
static qboolean
G_BuildablesIntersect(buildable_t a, vec3_t originA, buildable_t b, vec3_t originB)
{
  vec3_t minsA, maxsA;
  vec3_t minsB, maxsB;
...
static int
G_CompareBuildablesForRemoval(const void *a, const void *b)
{
  int precedence[] =
  { BA_NONE,
  ...
  aMatches = G_BuildablesIntersect(cmpBuildable, cmpOrigin, buildableA->s.modelindex, buildableA->s.origin);
...
/*
 ...
 ===============
 */
static itemBuildError_t
G_SufficientBPAvailable(buildable_t buildable, vec3_t origin, int biteam)
{
  int i;
  int numBuildables = 0;
  int pointsYielded = 0;
  ...
  for(i = MAX_CLIENTS, ent = g_entities + i;i < level.num_entities;i++, ent++)
  {
    ...
    if (buildable == BA_H_REPEATER && buildable == ent->s.modelindex && Distance(ent->s.origin, origin) < REPEATER_BASESIZE)
    {
  ...
  qsort(level.markedBuildables, numBuildables, sizeof(level.markedBuildables[0]), G_CompareBuildablesForRemoval);

  // Determine if there are enough markees to yield the required BP
  for(;pointsYielded < buildPoints && level.numBuildablesForRemoval < numBuildables;level.numBuildablesForRemoval++)
  {
 ...
 */
static void
G_SetBuildableLinkState(qboolean link)
{
  int i;
  gentity_t *ent;
...
static void
G_SetBuildableMarkedLinkState(qboolean link)
{
  int i;
  gentity_t *ent;
...
G_CanBuild(gentity_t *ent, buildable_t buildable, int distance, vec3_t origin)
{
  ...
  if (ent && ent->client && g_ctn.integer > 0)
  {
    switch(buildable)
    {
      ...
      case BA_H_DCC:
      case BA_H_MEDISTAT:
        if (ent->ctn_build_count >= g_ctnbuildlimit.integer)
        {
          return IBE_PERMISSION;
        }
        break;
  ...
  {
    //human criteria
    if (!G_IsPowered(entity_origin, biteam))
    {
    ...
    if (BG_FindDCCTestForBuildable(buildable) && !G_IsDCCBuilt(biteam))
      reason = IBE_NODCC;
    ...
    if (buildable == BA_H_REPEATER)
    {
      ...
      if (i >= level.num_entities)
      {
        ...
        for(i = 1, tempent = g_entities + i;i < level.num_entities;i++, tempent++)
        {
          ...
          if (tempent->s.modelindex == BA_H_REPEATER && Distance(tempent->s.origin, entity_origin) < REPEATER_BASESIZE)
          {
            reason = IBE_RPTWARN2;
            break;
          }
      ...
      else if (G_IsPowered(entity_origin, biteam))
        reason = IBE_RPTWARN2;
    }

    //check permission to build here
    if (tr1.surfaceFlags & SURF_NOHUMANBUILD || tr1.surfaceFlags & SURF_NOBUILD || contents & CONTENTS_NOHUMANBUILD || contents & CONTENTS_NOBUILD)
    ...
    if (BG_FindUniqueTestForBuildable(buildable))
    {
      for(i = 1, tempent = g_entities + i;i < level.num_entities;i++, tempent++)
      {
        ...
        if (tempent->s.modelindex == BA_H_REACTOR && !tempent->deconstruct && (tempent->biteam == biteam))
        {
          reason = IBE_REACTOR;
          break;
        }
  ...
  if ((tempReason = G_SufficientBPAvailable(buildable, origin, biteam)) != IBE_NONE)
    reason = tempReason;
 ...
 */
static gentity_t *
G_Build(gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles)
{
  gentity_t *built;
  buildHistory_t *new;
  vec3_t normal;
  ...
  if (builder->client)
  {
    ...
    {
      built->biteam = built->s.modelindex2 = BIT_HUMANS;
    }
    if (builder->client->ps.stats[STAT_PTEAM] == PTE_ALIENS)
    {
      built->biteam = built->s.modelindex2 = BIT_ALIENS;
    }
  }
  ...
  if (!builder->client)
  {
    ...
    built->antispawncamp = level.time;
    ...
    VectorSet(built->s.pos.trDelta,0,0,0);
    ...
    built->s.pos.trType = TR_STATIONARY;
 ...
 */
static void
G_FinishSpawningBuildable(gentity_t *ent)
{
  trace_t tr;
  vec3_t dest;
  gentity_t *built;
...
G_CheckDBProtection(void)
{
  ...
  for(i = 1, ent = g_entities + i;i < level.num_entities;i++, ent++)
  {
    ...
    if ((!alienDBs && ent->biteam == BIT_ALIENS) || (!humanDBs && ent->biteam == BIT_HUMANS))
    {
...
G_LayoutSave(char *name)
{
  ...
  for(i = MAX_CLIENTS;i < level.num_entities;i++)
  {
    ...
    s = va("%i %f %f %f %f %f %f %f %f %f %f %f %f %d %d\n", ent->s.modelindex, ent->s.pos.trBase[0], ent->s.pos.trBase[1], ent->s.pos.trBase[2],
        ent->s.angles[0], ent->s.angles[1], ent->s.angles[2], ent->s.origin2[0], ent->s.origin2[1], ent->s.origin2[2], ent->s.angles2[0], ent->s.angles2[1],
        ent->s.angles2[2], ent->biteam, ent->survivalStage);
...
static void
G_LayoutBuildItem(buildable_t buildable, vec3_t origin, vec3_t angles, vec3_t origin2, vec3_t angles2, int biteam, int survivalStage)
{
  gentity_t *builder;
...
G_LayoutLoad(void)
{
  ...
  *(layout + len) = '\0';
  ...
  while(*layout)
  {
    ...
    {
      i = 0;
      sscanf(line, "%d %f %f %f %f %f %f %f %f %f %f %f %f %d %d\n", &buildable, &origin[0], &origin[1], &origin[2], &angles[0], &angles[1], &angles[2],
          &origin2[0], &origin2[1], &origin2[2], &angles2[0], &angles2[1], &angles2[2], &biteam, &survivalStage);
