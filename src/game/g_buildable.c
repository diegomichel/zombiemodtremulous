...
================
 */
qboolean G_IsPowered(vec3_t origin, int biteam) {
  gentity_t dummy;
...
================
 */
qboolean G_IsDCCBuilt(int biteam) {
  gentity_t dummy;
...
void HReactor_Think(gentity_t *self) {
  ...
  if (self->spawned && (self->health > 0)) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (enemy->client && (enemy->client->ps.stats[ STAT_PTEAM ] != team)) {
        ...
        G_SelectiveRadiusDamage(self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE,
                REACTOR_ATTACK_RANGE, self, MOD_REACTOR, team);
    ...
    if (self->health < self->lastHealth &&
            level.time > level.humanBaseAttackTimer && G_IsDCCBuilt(self->biteam)) {
...
void HArmoury_Activate(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->spawned) {
    ...
    if ((activator->client->ps.stats[ STAT_PTEAM ] != PTE_HUMANS && activator->client->ps.stats[ STAT_PTEAM ] != PTE_ALIENS) || (activator->client->ps.stats[ STAT_PTEAM ] != team))
      return;
...
void HMGTurret_FindEnemy(gentity_t *self) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if ((target->biteam != self->biteam) || (target->client->ps.stats[ STAT_PTEAM ] != team)) {
      //if target is not valid keep searching
...
itemBuildError_t G_CanBuild(gentity_t *ent, buildable_t buildable, int distance, vec3_t origin) {
  ...
  if (ent && ent->client && g_ctn.integer > 0) {
    switch (buildable) {
      ...
      case BA_H_REACTOR:
      case BA_H_REPEATER:
      case BA_H_SPAWN:return IBE_PERMISSION;
      default:return IBE_PERMISSION;
    }
  }
